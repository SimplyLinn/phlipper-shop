...
class UntarWorker {
  onmessage(msg: MessageEvent) {
    try {
      ...
      this.postError(ensureError(thrown));
    }
  }
  postError(err: { message: string }) {
    this.postMessage({ type: 'error', data: { message: err.message } });
  }
  postLog(level: string, msg: string) {
  ...
  untarBuffer(arrayBuffer: ArrayBuffer) {
    try {
      ...
      this.postError(ensureError(err));
    }
  }
...
function decodeUTF8(bytes: ArrayLike<number>) {
  let s = '';
  let i = 0;
...
interface PaxHeaderField<Key extends keyof TarFile = keyof TarFile> {
  name: Key;
  value: TarFile[Key] | null;
}

class PaxHeader {
  constructor(fields: PaxHeaderField[]);
  constructor(private _fields: PaxHeaderField[]) {}
  ...
  static parse(buffer: ArrayBufferLike) {
    ...
    while (bytes.length > 0) {
      ...
      const fieldName = fieldMatch[1] as keyof TarFile;
...
class LongFieldHeader<Key extends keyof TarFile> {
  constructor(fieldName: Key, fieldValue: TarFile[Key]);
  constructor(
    private _fieldName: Key,
    private _fieldValue: TarFile[Key],
  ) {}

  static parse(fieldName: keyof TarFile, buffer: ArrayBufferLike) {
...
class UntarFileStream {
  ...
  private _globalHeader: PaxHeader | null;
  ...
  private _readNextFile() {
    ...
    switch (file.type) {
      ...
      case 'K':
        isHeaderFile = true;
        header = LongFieldHeader.parse(
          'linkname',
          stream.readBuffer(file.size),
        );
        break;
    ...
    if (this._globalHeader !== null) {
      this._globalHeader.applyHeader(file);
    }
